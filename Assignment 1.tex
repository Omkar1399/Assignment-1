...
\begin{lstlisting}
https://github.com/Omkar1399/Assignment-1/tree/main/codes
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/Omkar1399/Assignment-1/blob/main/codes/cdf_plot.py
\end{lstlisting}
\begin{figure}
\centering
...
\[
\begin{cases}
\frac{1}{1-0}&\text{for $x\in[0,1]$}\\
0&\text{otherwise}\\
\end{cases}
\]
Cumulative density function:
\begin{align*}
F_{U}(x) = f_{U}(x) &= \int_{-\infty}^{\infty}f_{U}(x)(dx)
\\
&= \int_{0}^{x}f_{U}(x)(dx)
\\
&= \int_{0}^{x}\frac{1}{1-0}(dx)
\\
&= \frac{1}{1-0}[x]_{0}^{x}
\\
&= \frac{x-0}{1}
...
\begin{align*}
& E[u]=\int_{0}^{1} xF(x)dx
\\
& =\frac{1}{1-0}\int_{0}^{1}xdx
\\
& =\frac{[x^2]_{0}^{1}}{•2}
\\
& =0.5
\\
& E[u^2]=\int_{0}^{1} x^2F(x)dx
\\
& = \frac{1}{1-0}\int_{0}^{1}x^2dx
\\
& = \frac{[x^3]_{0}^{1}}{•3}
\\
& =\frac{1}{•3}
\end{align*}
Now substituting values of $E[U]$ and $E[U^2]$
\begin{align*}
